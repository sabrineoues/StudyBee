...
export function DashboardPage() {
  ...
  const [phoneDistractionsToday, setPhoneDistractionsToday] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!userService.isSignedIn()) return;
      try {
        const res = await visionService.getPhoneEventsToday();
        if (!alive) return;
        setPhoneDistractionsToday(typeof res.count === "number" ? res.count : null);
      } catch {
        if (!alive) return;
        setPhoneDistractionsToday(null);
      }
    })();
    return () => {
      alive = false;
            ...
            <div className="relative flex h-32 w-32 flex-shrink-0 items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <div className="mt-2 text-center">
                  <div className="text-lg font-bold">{phoneDistractionsToday ?? "—"}</div>
                  <div className="text-[10px] uppercase tracking-widest opacity-60">
                    {t("dashboard.phoneDistractionsToday")}
                  </div>
                </div>
              </div>
            </div>
