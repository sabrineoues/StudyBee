...
function RequireGuest({ children }: { children: React.ReactElement }) {
  return userService.isSignedIn() ? <Navigate to="/journal" replace /> : children;
}

function hasJournalEntryToday(createdAt: string) {
...
function RequireJournalComplete({ children }: { children: React.ReactElement }) {
  const location = useLocation();
  const [status, setStatus] = useState<'checking' | 'allowed' | 'needs-journal' | 'needs-login'>('checking');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const accessToken = localStorage.getItem('access') ?? userService.getAccessToken();
        const response = await axios.get('http://127.0.0.1:8000/api/journal/history/', {
          headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : undefined,
        });

        const history = Array.isArray(response.data) ? response.data : [];
        const hasToday = history.some((entry: { created_at?: string }) => {
          if (!entry?.created_at) return false;
          return hasJournalEntryToday(entry.created_at);
        });

        if (!alive) return;
        setStatus(hasToday ? 'allowed' : 'needs-journal');
      } catch {
        if (!alive) return;
        // On API failure, keep access open to avoid locking users out.
        setStatus('allowed');
      }
    })();
  ...
  if (status === 'needs-journal') return <Navigate to="/journal" replace />;
  return children;
}
