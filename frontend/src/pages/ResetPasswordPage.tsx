...
export function ResetPasswordPage() {
  ...
  async function onRequestEmail(e: FormEvent) {
    ...
    try {
      ...
      setStatus("success");
      setMessage(t("resetPassword.request.sent"));
    } catch (err) {
      ...
      setStatus("error");
      setMessage(msgFromApi ?? (typeof maybeAny?.message === "string" ? maybeAny.message : t("resetPassword.request.failed")));
    }
  }
  ...
  async function onSetNewPassword(e: FormEvent) {
    ...
    if (newPassword !== newPasswordConfirm) {
      setStatus("error");
      setMessage(t("resetPassword.confirm.mismatch"));
      return;
    }
    ...
    try {
      ...
      setStatus("success");
      setMessage(t("resetPassword.confirm.success"));
      ...
      setStatus("error");
      setMessage(msgFromApi ?? (typeof maybeAny?.message === "string" ? maybeAny.message : t("resetPassword.confirm.failed")));
    }
  }

  const title = hasLinkParams ? t("resetPassword.confirm.title") : t("resetPassword.request.title");
  const subtitle = hasLinkParams ? t("resetPassword.confirm.subtitle") : t("resetPassword.request.subtitle");
        ...
        <div className="grid w-full max-w-6xl grid-cols-1 items-center gap-12 lg:grid-cols-12">
          <div className="relative hidden pr-12 lg:col-span-7 lg:block">
            ...
            <h1 className="font-headline mb-8 text-[4rem] font-extrabold leading-[1.1] tracking-tighter text-on-surface">
              ...
              <br />
              <span className="text-primary italic">{t("resetPassword.heroTitle2")}</span>
            </h1>
              ...
              {hasLinkParams ? (
                <form className="space-y-6" onSubmit={onSetNewPassword}>
                  <div className="space-y-2">
                    ...
                    >
                      {t("resetPassword.confirm.newPassword")}
                    </label>
                    <input
                      id="new-password"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {t("resetPassword.confirm.confirmPassword")}
                      </label>
                      <button
                        type="button"
                        onClick={() => setShowPassword((v) => !v)}
                        aria-label={showPassword ? t("resetPassword.confirm.hidePassword") : t("resetPassword.confirm.showPassword")}
                      ...
                      >
                        {showPassword ? t("resetPassword.confirm.hide") : t("resetPassword.confirm.show")}
                      </button>
                    </div>
                    <input
                  ...
                  >
                    {status === "loading" ? t("resetPassword.confirm.saving") : t("resetPassword.confirm.submit")}
                  </button>
                ...
                <form className="space-y-6" onSubmit={onRequestEmail}>
                  <div className="space-y-2">
                    <label
                      ...
                      htmlFor="email"
                    >
                      {t("resetPassword.request.emailLabel")}
                    </label>
                    <input
                      id="email"
                      type="email"
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-full bg-gradient-primary py-4 font-bold text-white shadow-lg transition-all duration-200 hover:scale-[1.02] active:scale-95"
                  >
                    {status === "loading" ? t("resetPassword.request.sending") : t("resetPassword.request.submit")}
                  </button>

                  <div className="mt-8 text-center">
                    <p className="font-medium text-on-surface-variant">
                      {t("resetPassword.request.remembered")} {" "}
                      <Link to="/sign-in" className="ml-1 font-bold text-primary hover:underline">
                        {t("resetPassword.request.signIn")}
                      </Link>
                    </p>
                  </div>
                </form>
