...
def generate_quiz(request):
    session_id = request.data.get("session_id", "default")
    result     = generate_with_rag(session_id, "quiz de révision du document", "quiz")
    save_exchange(get_or_create_session(session_id), "Quiz", result)
    return Response({"result": result})
...
def save_chat_message(request):
    ...
    session = get_or_create_session(session_id)
    msg = Message.objects.create(chat_session=session, role=role, content=content)
