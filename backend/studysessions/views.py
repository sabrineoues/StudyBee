...
from .models import StudySession, StudySessionTask
from .serializers import (
    AdminStudySessionSerializer,
    StudySessionSerializer,
    StudySessionTaskSerializer,
)
...
class StudySessionListCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if payload.get("study_duration") in (None, ""):
            payload["study_duration"] = payload.get("ai_recommended_duration") or DEFAULT_STUDY_DURATION
        if payload.get("break_duration") in (None, ""):
            payload["break_duration"] = DEFAULT_BREAK_DURATION
        if payload.get("focusScore") in (None, ""):
            payload["focusScore"] = 0
        if payload.get("streakscore") in (None, ""):
            payload["streakscore"] = 0

        serializer = StudySessionSerializer(data=payload)
...
class StudySessionTaskListCreateAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get_session(self, request, session_id: int):
        return StudySession.objects.filter(id=session_id, user=request.user).first()
    ...
    def get(self, request, session_id: int):
        ...
        return Response(StudySessionTaskSerializer(tasks, many=True).data, status=status.HTTP_200_OK)
    ...
    def post(self, request, session_id: int):
        ...
        serializer = StudySessionTaskSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        task = serializer.save(session=session)
        return Response(StudySessionTaskSerializer(task).data, status=status.HTTP_201_CREATED)


class StudySessionTaskDetailAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get_object(self, request, session_id: int, task_id: int):
        return StudySessionTask.objects.filter(
            id=task_id,
            session__id=session_id,
            session__user=request.user,
        ).first()

    def patch(self, request, session_id: int, task_id: int):
        task = self.get_object(request, session_id, task_id)
        if not task:
            return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)

        serializer = StudySessionTaskSerializer(task, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        task = serializer.save()
        return Response(StudySessionTaskSerializer(task).data, status=status.HTTP_200_OK)

    def delete(self, request, session_id: int, task_id: int):
        task = self.get_object(request, session_id, task_id)
        if not task:
            return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)
