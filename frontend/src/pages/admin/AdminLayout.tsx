...
export function AdminLayout() {
  ...
  const displayName = useMemo(() => {
    ...
    return full || user?.username || t("admin.layout.admin");
  }, [user?.first_name, user?.last_name, user?.username, currentLanguage, t]);
        ...
        <aside className="w-72 shrink-0 self-start md:sticky md:top-10">
          <div className="rounded-3xl bg-surface-container-low/90 p-6 shadow-sm ring-1 ring-outline-variant/12 backdrop-blur-md">
            ...
            <nav className="flex flex-col gap-2">
              ...
              <SideLink to="/admin/sessions" label={t("admin.layout.sessions")} />
              <SideLink to="/admin/journals" label={t("admin.layout.journals")} />
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-6 flex items-center justify-end gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-surface-container-low/90 px-4 py-2 text-sm font-semibold text-on-surface shadow-sm ring-1 ring-outline-variant/12 backdrop-blur-md transition-transform transition-colors duration-200 hover:scale-105 hover:bg-surface-container-low active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
            >
              <span className="material-symbols-outlined text-[20px] leading-none" aria-hidden="true">
                language
              </span>
              <span className="text-xs font-black tracking-widest text-on-surface/80">
